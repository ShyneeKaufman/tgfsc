...
export class MapView {
  ...
  renderBiomes() {
    return BIOMES.map(biome => {
      ...
      let actionHTML = '';
      if (isCurrent) {
        actionHTML = `<button class="btn-biome-action current" disabled>⚓ ВЫ ЗДЕСЬ</button>`;
      } else if (isUnlocked) {
        ...
        actionHTML = `<button class="btn-biome-action locked" disabled>🔒 ТРЕБУЕТСЯ ${biome.requiredLevel} УРОВЕНЬ</button>`;
      } else {
        ...
        actionHTML = `
          <button class="btn-biome-action unlock-btn ${canAfford ? '' : 'cant-afford'}" data-action="unlock" data-id="${biome.id}" ${canAfford ? '' : 'disabled'}>
            ОТКРЫТЬ: ${biome.travelCost.toLocaleString('ru-RU')}🪙
          </button>
        `;
      }
        ...
        <div class="biome-card ${isCurrent ? 'current-biome' : ''}" style="background: linear-gradient(145deg, ${biome.skyGradient[0]}, ${biome.skyGradient[1]})">
          ...
          <div class="biome-card-content">
            <div class="biome-header-row">
              <span class="biome-big-icon">${biome.icon}</span>
              <div class="biome-title-group">
                <h3 class="biome-title">${biome.name}</h3>
                <span class="biome-req-level">Мин. уровень: ${biome.requiredLevel}</span>
              </div>
            </div>
